...
const ProductsView = () => {
  ...
  const transformedProducts =
    products?.map((product, index) => transformProduct(product, index)) || [];
  ...
  if (error) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>
          Failed to load products. Please make sure the backend server is
          running.
        </AlertDescription>
      </Alert>
    );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          ...
          <div className="bg-card shadow-soft rounded-2xl p-4 sm:p-6">
            ...
            <div className="text-2xl sm:text-3xl font-bold text-yellow-600">
              {
                transformedProducts.filter((p) => p.status === "low-stock")
                  .length
              }
            </div>
          </div>
          <div className="bg-card shadow-soft rounded-2xl p-4 sm:p-6">
            ...
            <div className="text-2xl sm:text-3xl font-bold text-red-600">
              {
                transformedProducts.filter((p) => p.status === "out-of-stock")
                  .length
              }
            </div>
          </div>
